        ...
        function($, undefined) {
            ...
            $.each(vendors,
                ...
                $.fn.animate = function(properties, duration, ease, callback, delay) {
                    return $.isFunction(duration) && (callback = duration, ease = undefined, duration = undefined),
                        $.isFunction(ease) && (callback = ease, ease = undefined),
                        $.isPlainObject(duration) && (ease = duration.easing, callback = duration.complete, delay = duration.delay, duration = duration.duration),
                        duration && (duration = ("number" == typeof duration ? duration: $.fx.speeds[duration] || $.fx.speeds._default) / 1e3),
                        delay && (delay = parseFloat(delay) / 1e3),
                        this.anim(properties, duration, ease, callback, delay)
                },
                $.fn.anim = function(properties, duration, ease, callback, delay) {
                    var key,
                        ...
                        fired = !1;
                    if (duration === undefined && (duration = $.fx.speeds._default / 1e3), delay === undefined && (delay = 0), $.fx.off && (duration = 0), "string" == typeof properties) cssValues[animationName] = properties,
                        ...
                        transforms && (cssValues[transform] = transforms, cssProperties.push(transform)),
                            duration > 0 && "object" == typeof properties && (cssValues[transitionProperty] = cssProperties.join(", "), cssValues[transitionDuration] = duration + "s", cssValues[transitionDelay] = delay + "s", cssValues[transitionTiming] = ease || "linear")
                    }
                    return wrappedCallback = function(event) {
                        ...
                        this.css(cssValues),
                        0 >= duration && setTimeout(function() {
                            that.each(function() {
        ...
        function($) {
            $.fn.coffee = function(option) {
                function steam() {
                    ...
                    left > position && (left = rand(0, position)),
                        $fly.css({
                            position: "absolute",
                            left: position,
                            ...
                            rand(opts.steamFlyTime / 2, 1.2 * opts.steamFlyTime), __flyFastSlow,
                            function() {
                                $fly.remove(),
                                    $fly = null
                            })
                ...
                function randoms(length, chars) {
                    ...
                    var hash = "",
                        ...
                        num = 0;
                    for (i = 0; length > i; i++) num = rand(0, maxNum - 1),
                ...
                return $.fn.coffee.stop = function() {
                    ...
                    $.fn.coffee.start = function() {
                        __time_val = setInterval(function() {
                            ...
                            rand(opts.steamInterval / 2, 2 * opts.steamInterval)),
